...
namespace model
{
    public class Database
    {
        ...
        public StringBuilder JobScript = new StringBuilder();
...
IF EXISTS (SELECT 1 FROM tempdb..sysobjects WHERE name = '##jobids')
DROP TABLE ##jobids
...
PRINT 'EXECUTE @ReturnCode = msdb.dbo.sp_add_jobserver @job_id = @JobID, @server_name = N''(local)''
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
";
                    try
                    {
                        using (SqlDataAdapter sda = new SqlDataAdapter())
                        {
                            ...
                            foreach (DataTable dt in ds.Tables)
                            {
                                ...
                                {
                                    JobScript.AppendLine(row[0].ToString());
                                }
                            }
                        }
                    }
        ...
        public void ScriptJobsToDir()
        {
            ...
            using (System.IO.StreamWriter fileWriter = new StreamWriter(Dir + @"\SQL_Agent_Jobs.sql", true))
            {
                fileWriter.Write(JobScript.ToString());
            }
